...
int main(int argc, char *argv[]) {
    ...
    for (step = 0; (step < maxsteps) && (maxdiff >= threshold); ++step) {
      ...
      if (myID != 0) 
        MPI_Isend(&uk[1], 1, MPI_DOUBLE, leftNbr, 0, MPI_COMM_WORLD,&request[0]);
      if (myID != numProcs-1)
        MPI_Isend(&uk[numPoints], 1, MPI_DOUBLE, rightNbr, 0, MPI_COMM_WORLD,&request[1]);
      if (myID != 0)
        MPI_Irecv(&uk[0], 1, MPI_DOUBLE, leftNbr, 0, MPI_COMM_WORLD, &request[2]);
      if (myID != numProcs-1)
        MPI_Irecv(&uk[numPoints+1],1, MPI_DOUBLE, rightNbr, 0, MPI_COMM_WORLD, &request[3]);
